...
class PrepareTarget:
    ...
    def data_normalize(
        ...
        target_time_points = ["1h", "3h", "6h", "10h", "16h", "24h", "34h", "48h"]

        if normalize_method == "MinMax":
            # Use MinMaxScaler to scale values to [0, 1]
            scaler = MinMaxScaler()
            for time_point in target_time_points:
                col_name = f"corrRatio_{time_point}"
                df[col_name] = scaler.fit_transform(df[[col_name]])

        elif normalize_method == "clip_arch":
            # Clip each time point's values within [0, 1]
            for time_point in target_time_points:
                label_loss_col = f"Label loss_{time_point}"
                label_incorp_col = f"Label incorporation_{time_point}"
                turnover_rate_col = f"TurnoverRate_{time_point}"

                df[label_loss_col] = df[label_loss_col].clip(lower=0, upper=1)
                df[label_incorp_col] = df[label_incorp_col].clip(lower=0, upper=1)
                df[turnover_rate_col] = df[turnover_rate_col].clip(lower=0, upper=1)
