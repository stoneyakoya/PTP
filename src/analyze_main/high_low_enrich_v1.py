...
def create_comparison_bubble_plot(
    ...
    out_dir: str = 'data/data_analyze/enrichment',
    ...
    cmap = plt.cm.RdYlBu_r  # 論文でよく使われるカラーマップ
    ...
    # 保存
    outfile = os.path.join(out_dir, 'cluster_comparison_enrichment_bubble.pdf')
    plt.savefig(outfile, dpi=600, bbox_inches='tight', facecolor='white', edgecolor='none', format='pdf')
    plt.show()
    print(f"\n保存完了: {outfile}")
...
def main():
    ...
    for cluster_id in clusters_to_analyze:
        ...
        if not enrichment_df.empty:
            ...
            print(enrichment_df[['name', 'p_value', 'intersection_size']][:20])
    ...
    if enrichment_results:
        ...
        create_comparison_bubble_plot(
            enrichment_results,
            out_dir='data/data_analyze/enrichment',
    ...
    #     title='Enrichment Analysis: Very Rapid Cluster (High vs Low)'
    # )
    # # Supplementary CSV保存
